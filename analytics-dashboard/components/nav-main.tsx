      ...
      <SidebarGroupContent>
        ...
        <SidebarMenu className="space-y-4">
          {navSections.map((section) => (
            <SidebarMenuItem key={section.id} className="mb-4">
              {section.items ? (
                // Section with submenu
                <>
                  <SidebarMenuButton
                    onClick={() => toggleSection(section.id)}
                    className="w-full justify-between mb-2 hover:bg-accent"
                  >
                  ...
                  {openSections.includes(section.id) && (
                    <SidebarMenuSub className="space-y-4 py-4 pl-4">
                      {section.items.map((item) => (
                        <SidebarMenuSubItem key={item.href}>
                          <SidebarMenuSubButton
                            className={cn(
                              "px-4 py-3 leading-relaxed w-full text-sm hover:bg-accent rounded-md",
                ...
                <SidebarMenuButton
                  ...
                  className={cn(
                    ...
                    section.href && isActive(section.href) && "bg-accent text-accent-foreground",
                  )}
                >
                  <section.icon className="mr-2 h-4 w-4" />
                  {section.label}
